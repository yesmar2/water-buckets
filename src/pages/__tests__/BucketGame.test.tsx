...
describe('<BucketGame>', () => {
  ...
  it('renders bucket one size', () => {
    setup();
    const bucketOneText = screen.getByText(`Size: ${mockBucketOneSize} unit(s)`);
    expect(bucketOneText).toBeInTheDocument();
  });

  it('renders bucket two size', () => {
    setup();
    const bucketTwoText = screen.getByText(`Size: ${mockBucketTwoSize} unit(s)`);
    expect(bucketTwoText).toBeInTheDocument();
  });

  it('clicking the fill button fills bucket', () => {
    setup();
    const bucketOneFillButton = screen.getAllByRole('button', { name: 'Fill'})[0];
  ...
  it('clicking the dump button dumps bucket', () => {
    ...
    const bucketOneDumpButton = screen.getAllByRole('button', { name: 'Dump'})[0];
    fireEvent.click(bucketOneDumpButton);
    expect(screen.queryByText(`${mockBucketOneSize} unit(s)`)).not.toBeInTheDocument();
  });
  ...
  it('clicking the transfer button transfers units correctly', () => {
    ...
    const bucketOneTransferButton = screen.getAllByRole('button', { name: 'Transfer'})[0];
  ...
  it('shows winner text when the game is won', () => {
    ...
    const bucketOneFillButton = screen.getAllByRole('button', { name: 'Fill'})[0];
    const bucketOneTransferButton = screen.getAllByRole('button', { name: 'Transfer'})[0];
    const bucketTwoDumpButton = screen.getAllByRole('button', { name: 'Dump'})[1];
    fireEvent.click(bucketOneFillButton);
    fireEvent.click(bucketOneTransferButton);
